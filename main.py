...
YANDEX_EMAIL = os.environ.get("YANDEX_EMAIL", "petr.klenyaev@yandex.ru")
...
# ========== API ЭНДПОИНТЫ (все с response_model=None) ==========
...
async def register(email: str = Form(...), nickname: str = Form(...), password: str = Form(...), code: str = Form(...)):
    db = SessionLocal()
    try:
...
async def websocket_endpoint(websocket: WebSocket, token: str):
    ...
    try:
        ...
        # Отправить историю (последние 50 сообщений)
